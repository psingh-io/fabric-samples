    ...
    try {
        ...
        const listener = await network.addBlockListener(
            async (event) => {
                ...
                // TODO: Additional filtering logic
                let txnEvents = event.getTransactionEvents();

                //event.getTransactionEvents()["0"].privateData.ns_pvt_rwset["0"].collection_pvt_rwset["1"].rwset.writes["0"].value.toString()

                for(let txnEvent of txnEvents) {
                    if (txnEvent.privateData) {
                        if (txnEvent.privateData.ns_pvt_rwset) {
                            for (let pivateRWSet of txnEvent.privateData.ns_pvt_rwset) {
                                if (pivateRWSet.collection_pvt_rwset) {
                                    for (let privateRWSetCollection of pivateRWSet.collection_pvt_rwset) {
                                        if (privateRWSetCollection.rwset && privateRWSetCollection.rwset.writes) {
                                            for (let privateWriteBuffer of privateRWSetCollection.rwset.writes) {
                                                // let s = JSON.stringify(privateWriteBuffer)
                                                // console.log('privateWriteBuffer', s)
                                                // let privateData = JSON.parse(s)
                                                // console.log(privateData);
                                                console.log('Block# ', event.blockNumber.toString())
                                                console.log('key', privateWriteBuffer.key)
                                                console.log('val', privateWriteBuffer.value.toString())
                                            }
                                        }
                                    }
                                }
